...
// Validation schema for bead: centralized-docs-20260402075248-nbxk7fwk
// Title: action: migrate watch/apply snapshot persistence from `DocCache` to `StateDb`
//
// This schema validates that implementation is complete.
// Use: cue vet centralized-docs-20260402075248-nbxk7fwk.cue implementation.cue

#BeadImplementation: {
  bead_id: "centralized-docs-20260402075248-nbxk7fwk"
  title: "action: migrate watch/apply snapshot persistence from `DocCache` to `StateDb`"
  ...
  contracts_verified: {
    ...
    precondition_checks: [
      "`StateDb` can store archived snapshots in the `snapshots` table.",
      "`cmd/watch.rs` currently isolates snapshot I/O in helper functions.",
    ]

    // Specific postconditions that must be verified
    postcondition_checks: [
      "`open_cache`, `load_snapshot`, and `store_snapshot` in `cmd/watch.rs` are replaced or reworked to use `StateDb`.",
    ...
    invariant_checks: [
      ...
      "The migration stays confined to the command-side I/O boundary.",
    ]
  }
  ...
  tests_passing: {
    ...
    required_happy_tests: [
      ...
      "A test verifies migrated snapshot helpers preserve existing snapshot lookup semantics for repeat runs.",
    ]
