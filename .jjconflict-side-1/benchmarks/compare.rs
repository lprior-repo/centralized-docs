...
fn main() {
    let haystack = "This is a regular text line without any copyright keyword inside it. ".repeat(10);
    
    let t = Instant::now();
    for _ in 0..100000 { contains_ignore_ascii_case(&haystack, "copyright"); }
    println!("contains_ignore_ascii_case: {:?}", t.elapsed());
    
    let t = Instant::now();
    for _ in 0..100000 { contains_lower(&haystack, "copyright"); }
    println!("contains_lower: {:?}", t.elapsed());
}
