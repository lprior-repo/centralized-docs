...
// Validation schema for bead: centralized-docs-20260402075248-m86iecjh
// Title: data: add fixed-size `FileStateRaw` and `UrlStateRaw` pod types
//
// This schema validates that implementation is complete.
// Use: cue vet centralized-docs-20260402075248-m86iecjh.cue implementation.cue

#BeadImplementation: {
  bead_id: "centralized-docs-20260402075248-m86iecjh"
  title: "data: add fixed-size `FileStateRaw` and `UrlStateRaw` pod types"
  ...
  contracts_verified: {
    ...
    precondition_checks: [
      ...
      "The new raw state module can be referenced from production code.",
    ]
  ...
  tests_passing: {
    ...
    required_happy_tests: [
      ...
      "A unit test verifies `bytemuck::bytes_of` and `pod_read_unaligned` round-trip both raw structs without layout surprises.",
    ]

    // Required error path tests
    required_error_tests: [
      "Compilation fails if a non-Pod field is accidentally added to either raw struct.",
      "A layout assertion test fails if a field change breaks the documented fixed-size footprint.",
    ]
  }
