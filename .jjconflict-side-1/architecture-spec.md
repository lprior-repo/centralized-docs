...
```
STARTUP:  let read = state_db.begin_read()?;
          bulk load url_state → HashMap<String, UrlStateRaw>
...
LOAD:     read.load_scrapes(scrape_hashes_for_unchanged_urls)
          → HashMap<[u8; 32], OwnedArchive<ScrapedPage>>
PROCESS:  only process CHANGED + NEW pages
          UNCHANGED pages: reuse cached `ScrapedPage` from redb
          drop(read) before starting the write transaction
...
Already uses redb for snapshots. Migrate `DocCache` calls to `StateDb` so watch/apply opens the same `StateReadSession`, calls explicit snapshot loaders during startup, then writes `new_snapshots` and `deleted_snapshots` through the single shutdown write transaction.
...
| Archived return type | `OwnedArchive<T>` | Keeps archived bytes owned and usable across the run without exposing transaction-scoped archived values. |
| Read transaction model | `StateReadSession` | One shared read transaction per command, then one write transaction after the read session is dropped. |
