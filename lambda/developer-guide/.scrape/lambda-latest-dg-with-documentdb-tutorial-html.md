---
url: https://docs.aws.amazon.com/lambda/latest/dg/with-documentdb-tutorial.html
title: Tutorial: Using AWS Lambda with Amazon DocumentDB Streams
word_count: 3528
filtered: true
elements_removed: 0
density_score: 0.87
...
Tutorial: Using AWS Lambda with Amazon DocumentDB Streams - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#with-documentdb-tutorial)
[Create the cluster](#docdb-documentdb-cluster)[Create the secret in Secrets Manager](#docdb-secret-in-secrets-manager)[Connect to the cluster](#docdb-connect-to-cluster)[Activate change streams](#docdb-activate-change-streams)[Create interface VPC endpoints](#docdb-create-interface-vpc-endpoints)[Create the execution role](#docdb-create-the-execution-role)[Create the Lambda function](#docdb-create-the-lambda-function)[Create the Lambda event source mapping](#docdb-create-the-lambda-event-source-mapping)[Test your function](#docdb-test-insert)[Troubleshooting](#docdb-lambda-troubleshooting)[Clean up your resources](#docdb-cleanup)
...
1. Open the [Secrets Manager](https://console.aws.amazon.com/secretsmanager/home#) console and choose
**Store a new secret**.
...
* Choose **Next**.
* For **Configure secret**, choose the following options:
* **Secret name**: `DocumentDBSecret`
* Choose **Next**.
* Choose **Next**.
* Choose **Store**.
* Refresh the console to verify that you successfully stored the `DocumentDBSecret` secret.
...
in your Amazon DocumentDB cluster. You do this by activating [change streams](https://docs.aws.amazon.com/documentdb/latest/developerguide/change_streams.html).
...
1. Open the [VPC console](https://console.aws.amazon.com/vpc/home#). In the left menu, under **Virtual private cloud**,
choose **Endpoints**.
...
* Again, choose **Create endpoint**. Create an endpoint with the following configuration:
...
1. Open the [Roles page](https://console.aws.amazon.com/iam/home#/roles) in the IAM console and choose
**Create role**.
2. For **Select trusted entity**, choose the following options:
* **Trusted entity type**: AWS service
* **Service or use case**: Lambda
* Choose **Next**.
* For **Add permissions**, choose the `AWSDocumentDBLambdaPolicy` policy you just created,
as well as the `AWSLambdaBasicExecutionRole` to give your function permissions to write to Amazon CloudWatch Logs.
* Choose **Next**.
* For **Role name**, enter `AWSDocumentDBLambdaExecutionRole`.
* Choose **Create role**.
...
* Choose **Add**. Creating your event source mapping can take a few minutes.
## Test your function
...
```
Verify that your function successfully processed this event by [checking CloudWatch Logs](./monitoring-cloudwatchlogs-view.html#monitoring-cloudwatchlogs-console). You should see a log entry like this:
![CloudWatch log stream for database record insertion](https://docs.aws.amazon.com/images/lambda/latest/dg/images/documentdb-insert-log.png)
...
```
Verify that your function successfully processed this event by [checking CloudWatch Logs](./monitoring-cloudwatchlogs-view.html#monitoring-cloudwatchlogs-console). You should see a log entry like this:
![CloudWatch log stream for database record update](https://docs.aws.amazon.com/images/lambda/latest/dg/images/documentdb-update-log.png)
Delete the record that you just updated with the following command:
```
`db.products.deleteOne( { "name": "Pencil" } )`
```
Verify that your function successfully processed this event by [checking CloudWatch Logs](./monitoring-cloudwatchlogs-view.html#monitoring-cloudwatchlogs-console). You should see a log entry like this:
![CloudWatch log stream for database record deletion](https://docs.aws.amazon.com/images/lambda/latest/dg/images/documentdb-delete-log.png)
## Troubleshooting
...
![Lambda event source mapping details](https://docs.aws.amazon.com/images/lambda/latest/dg/images/documentdb-trigger.png)
...
![Lambda event source mapping details](https://docs.aws.amazon.com/images/lambda/latest/dg/images/documentdb-lastprocessingresult.png)
