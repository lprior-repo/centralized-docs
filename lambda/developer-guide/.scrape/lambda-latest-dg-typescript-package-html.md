---
url: https://docs.aws.amazon.com/lambda/latest/dg/typescript-package.html
title: Deploy transpiled TypeScript code in Lambda with .zip file archives
word_count: 1654
filtered: true
elements_removed: 0
density_score: 0.89
...
Deploy transpiled TypeScript code in Lambda with .zip file archives - AWS Lambda
Deploy transpiled TypeScript code in Lambda with .zip file archives - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#typescript-package)
[Using AWS SAM](#aws-sam-ts)[Using the AWS CDK](#aws-cdk-ts)[Using the AWS CLI and esbuild](#aws-cli-ts)
...
```
2. (Optional) The sample application includes configurations for commonly used tools, such as [ESLlint](https://eslint.org/) for code linting and [Jest](https://jestjs.io/) for unit testing. To run lint and test commands:
```
...
```
3. Add the [@types/aws-lambda](https://www.npmjs.com/package/@types/aws-lambda) package as a development dependency. This package contains the type definitions for Lambda.
```
...
* Open **hello-world-stack.ts**. This is the code that defines your [AWS CDK stack](https://docs.aws.amazon.com/cdk/v2/guide/stacks.html). Replace the code with the following:
```
...
```
* from the `hello-world` directory containing your `cdk.json` file, deploy your
application.
```
`cdk deploy`
```
...
* [AWS CLI version 2](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html)
* Node.js
* An [execution role](./lambda-intro-execution-role.html) for the Lambda function
...
`npm init`
```
3. Add the [@types/aws-lambda](https://www.npmjs.com/package/@types/aws-lambda) and [esbuild](https://esbuild.github.io/) packages as development dependencies. The `@types/aws-lambda` package contains the type definitions for Lambda.
```
...
```
5. Add a build script to the **package.json** file. This configures esbuild to automatically create the .zip deployment package. For more information, see [Build scripts](https://esbuild.github.io/getting-started/#build-scripts) in the esbuild documentation.
Linux and MacOS
```
`"scripts": {
"prebuild": "rm -rf dist",
...
```
Windows
In this example, the `"postbuild"` command uses the [7zip](https://www.7-zip.org/download.html) utility to
...
`npm run build`
```
7. Create a Lambda function using the .zip deployment package. Replace the highlighted text with the Amazon Resource Name (ARN) of your [execution role](./lambda-intro-execution-role.html).
```
...
```
8. [Run a test event](./testing-functions.html) to confirm that the function returns the following response. If you want to invoke this function using API Gateway, [create and configure a REST API](https://docs.aws.amazon.com/apigateway/latest/developerguide/how-to-create-api.html).
```
`{
"statusCode": 200,
