---
url: https://docs.aws.amazon.com/lambda/latest/dg/monitoring-cloudwatchlogs-view.html
title: Viewing CloudWatch logs for Lambda functions
word_count: 2482
filtered: true
elements_removed: 0
density_score: 0.87
...
Viewing CloudWatch logs for Lambda functions - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#monitoring-cloudwatchlogs-view)
[Stream function logs with CloudWatch Logs Live Tail](#monitoring-live-tail)[Access function logs using the console](#monitoring-cloudwatchlogs-console)[Access logs with the AWS CLI](#monitoring-cloudwatchlogs-cli)[Parsing logs and structured logging](#querying-logs)[Log visualization and dashboards](#monitoring-logs-visualization)
...
###### Note
[Lambda Managed Instances](./lambda-managed-instances.html) does not support the `--log-type Tail` option. Use CloudWatch Logs Live Tail or query CloudWatch Logs directly to view logs for Managed Instances functions.
### Permissions
...
![monitoring observability figure 3](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-3.png)
These logs show:
...
[glob](https://en.wikipedia.org/wiki/Glob_(programming)) and
...
![monitoring observability figure 10](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-10.png)
...
![monitoring observability figure 11](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-11.png)
...
|filteruploadedBytes &gt;&gt; 1000000
| filter uploadTimeMS &gt;&gt; 1000
| filter invocation != 1`
```
This query might produce the following result:
![monitoring observability figure 12](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-12.png)
The discovered fields in JSON are automatically populated in the *Discovered fields*
...
```
filter @type="REPORT"
| stats avg(@maxMemoryUsed/1024/1024) as mean\_MemoryUsed,
min(@maxMemoryUsed/1024/1024) as min\_MemoryUsed,
max(@maxMemoryUsed/1024/1024) as max\_MemoryUsed,
...
[
visualizations from queries](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/CWL_Insights-Visualizing-Log-Data.html), providing there is at least one aggregation function. The `stats`
...
[JSON structured logging](#querying-logs-json) section, but with additional aggregation functions:
```
`fields@message
|filter@messagelike/INFO/
|filteruploadedBytes&gt;&gt;1000000
|filteruploadTimeMS&gt;&gt;1000
|filterinvocation!=1
|statsmin(uploadedBytes),avg(uploadedBytes),max(uploadedBytes)bybin(1m)`
```
...
![monitoring observability figure 14](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-14.png)
...
![monitoring observability figure 15](https://docs.aws.amazon.com/images/lambda/latest/dg/images/monitoring-observability-figure-15.png)
