---
url: https://docs.aws.amazon.com/lambda/latest/dg/runtime-management-rollback.html
title: Rolling back a Lambda runtime version
word_count: 614
filtered: true
elements_removed: 0
density_score: 0.80
...
Rolling back a Lambda runtime version - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#runtime-management-rollback)
[Roll back a runtime version using Manual runtime update mode](#runtime-management-rollback-manual)[Roll back a runtime version using published function versions](#runtime-management-rollback-published)
# Rolling back a Lambda runtime version
In the rare event that a new runtime version is incompatible with your existing function,
...
runtime management controls](https://aws.amazon.com/blogs/compute/introducing-aws-lambda-runtime-management-controls/) on the AWS Compute Blog.
...
using the `$LATEST` runtime version, you can roll back your runtime version using
the **Manual** mode. For the [function version](./configuration-versions.html) you want to roll back, change the runtime version update mode to
**Manual** and specify the ARN of the previous runtime version. For more
information about finding the ARN of the previous runtime version, see [Identifying Lambda runtime version changes](./runtime-management-identify.html).
###### Note
If the `$LATEST` version of your function is configured to use
...
Published [function versions](./configuration-versions.html) are an
immutable snapshot of the `$LATEST` function code and configuration at the time
...
Published function versions using **Function update** mode therefore
...
see [Understanding the shared responsibility model for Lambda runtime management](./runtime-management-shared.html).
