...
Designing Lambda applications - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#concepts-application-design)
[Use services instead of custom code](#services-custom-code)[Understand Lambda abstraction levels](#level-abstraction)[Implement statelessness in functions](#statelessness-functions)[Minimize coupling](#minimize-coupling)[Build for on-demand data instead of batches](#on-demand-batches)[Choose an orchestration option for complex workflows](#orchestration)[Implement idempotency](#retries-failures)[Use multiple AWS accounts for managing quotas](#multiple-accounts)
# Designing Lambda applications
A well-architected event-driven application uses a combination of AWS services and custom code to process
and manage requests and data. This chapter focuses on Lambda-specific topics in application design. There are
many important considerations for serverless architects when designing applications for busy production systems.
Many of the best practices that apply to software development and distributed systems also apply to serverless
application development. The overall goal is to develop workloads that are:
* **Reliable** – offering your end users a high level of availability.
AWS serverless services are reliable because they are also designed for failure.
* **Durable** – providing storage options that meet the durability
needs of your workload.
* **Secure** – following best practices and using the tools provided
to secure access to workloads and limit the blast radius.
* **Performant** – using computing resources efficiently and meeting
...
Serverless applications usually comprise several AWS services, integrated with custom code run in Lambda
functions. While Lambda can be integrated with most AWS services, the services most commonly used in serverless
applications are:
...
There are many well-established, common patterns in distributed architectures that you can build yourself
or implement using AWS services. For most customers, there is little commercial value in investing time
...
The Lambda service limits your access to the underlying operating systems, hypervisors, and hardware
running your Lambda functions. The service continuously improves and changes infrastructure to add features,
reduce cost and make the service more performant. Your code should assume no knowledge of how Lambda is
architected and assume no hardware affinity.
...
This abstraction helps you focus on the integration aspects of your application, the flow of data, and
the business logic where your workload provides value to your end users. Allowing the services to manage the
underlying mechanics helps you develop applications more quickly with less custom code to maintain.
...
global scope.
## Minimize coupling
Most architectures should prefer many, shorter functions over fewer, larger ones. The purpose of each
function should be to handle the event passed into the function, with no knowledge or expectations of the
overall workflow or volume of transactions. This makes the function agnostic to the source of the event with
...
[by using scheduled
expressions](https://docs.aws.amazon.com/eventbridge/latest/userguide/scheduled-events.html) for rules in Amazon EventBridge, these should be used sparingly or as a last-resort. In any scheduled
task that processes a batch, there is the potential for the volume of transactions to grow beyond what can be
...
![event driven architectures figure 10](https://docs.aws.amazon.com/images/lambda/latest/dg/images/event-driven-architectures-figure-10.png)
...
![event driven architectures figure 11](https://docs.aws.amazon.com/images/lambda/latest/dg/images/event-driven-architectures-figure-11.png)
...
* **[Lambda durable functions](./durable-functions.html):** Application-centric orchestration
...
* **[AWS Step Functions](./with-step-functions.html):** Visual workflow orchestration with
...
AWS serverless services, including Lambda, are fault-tolerant and designed to handle failures. For example, if a
...
An effective way to solve this issue is to use multiple AWS accounts, dedicating each workload to its own account.
This prevents quotas from being shared with other workloads or non-production resources.
...
![application design figure 3](https://docs.aws.amazon.com/images/lambda/latest/dg/images/application-design-figure-3.png)
...
machines against live cloud resources in their individual accounts.
