...
title: Manage Lambda function versions
word_count: 931
filtered: true
elements_removed: 0
density_score: 0.87
...
Manage Lambda function versions - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#configuration-versions)
[Creating function versions](#configuration-versions-config)[Using versions](#versioning-versions-using)[Granting permissions](#versioning-permissions)
...
* [Triggers](./lambda-services.html#lambda-invocation-trigger)
* [Destinations](./invocation-async-retain-records.html#create-destination)
* [Provisioned concurrency](./provisioned-concurrency.html)
* [Asynchronous invocation](./invocation-async.html)
* [Database connections and proxies](./services-rds.html#rds-configuration)
###### Note
When using [runtime management controls](./runtimes-update.html#runtime-management-controls) with **Auto**
...
For more information, see [Understanding how Lambda manages runtime version updates](./runtimes-update.html).
###### Sections
...
3. On the versions configuration page, choose **Publish new
version**.
...
```
###### Note
Lambda assigns monotonically increasing sequence numbers for versioning. Lambda never reuses version
...
If you decide not to publish function versions, you can invoke the function using either
the qualified or unqualified ARN in your [event
source mapping](./invocation-eventsourcemapping.html). When you invoke a function using an unqualified ARN, Lambda implicitly
invokes `$LATEST`.
The qualified ARN for each Lambda function version is unique. After you publish a version,
you can't change the ARN or the function code.
...
You can use a [resource-based policy](./access-control-resource-based.html) or an [identity-based policy](./access-control-identity-based.html) to grant access to your function. The scope
of the permission depends on whether you apply the policy to a function or to one version of a function. For more
information about function resource names in policies, see [Fine-tuning the Resources and Conditions sections of policies](./lambda-api-permissions-ref.html).
...
more information, see [Create an alias for a Lambda function](./configuration-aliases.html).
