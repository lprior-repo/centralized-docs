...
title: Log and monitor Java Lambda functions
word_count: 3937
filtered: true
elements_removed: 0
density_score: 0.87
...
Log and monitor Java Lambda functions - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#java-logging)
[Creating a function that returns logs](#java-logging-output)[Using Lambda advanced logging controls with Java](#java-logging-advanced)[Implementing advanced logging with Log4j2 and SLF4J](#java-logging-log4j2)[Tools and libraries](#java-tools-libraries)[Using Powertools for AWS Lambda (Java) and AWS SAM for structured logging](#java-logging-sam)[Viewing logs in the Lambda console](#java-logging-console)[Viewing logs in the CloudWatch console](#java-logging-cwconsole)[Viewing logs using the AWS Command Line Interface (AWS CLI)](#java-logging-cli)[Deleting logs](#java-logging-delete)[Sample logging code](#java-logging-samples)
...
To output logs from your function code, you can use methods on [java.lang.System](https://docs.oracle.com/javase/8/docs/api/java/lang/System.html), or any logging
...
To output logs from your function code, you can use methods on [java.lang.System](https://docs.oracle.com/javase/8/docs/api/java/lang/System.html), or any logging
module that writes to `stdout` or `stderr`. The [aws-lambda-java-core](./java-package.html) library provides a logger class named `LambdaLogger` that you can access
...
"AWS\_EXECUTION\_ENV": "AWS\_Lambda\_java8",
"AWS\_LAMBDA\_FUNCTION\_MEMORY\_SIZE": "512",
...
}
CONTEXT:
{
...
filters your log outputs using the “level” key value pair in the JSON object described in [Using structured JSON log format with Java](#java-logging-advanced-JSON).
...
[Using Powertools for AWS Lambda (Java) and AWS SAM for structured logging](#java-logging-sam) and [Implementing advanced logging with Log4j2 and SLF4J](#java-logging-log4j2) to learn more.
...
want Lambda to send to CloudWatch Logs:
...
For cases where a customer function includes an impacted Log4j2 version, we have applied a change to the Lambda Java [managed runtimes](./lambda-runtimes.html) and [base container images](./java-image.html) that helps to mitigate the issues in CVE-2021-44228, CVE-2021-45046, and CVE-2021-45105.
...
code.
To add the request ID to your function's logs, use the appender in the [aws-lambda-java-log4j2](./java-package.html) library.
###### Example [src/main/resources/log4j2.xml](https://github.com/awsdocs/aws-lambda-developer-guide/tree/main/sample-apps/s3-java/src/main/resources/log4j2.xml)
– Appender configuration
```
`&lt;Configuration&gt;
&lt;Appenders&gt;
&lt;Lambda name="Lambda" format="${env:AWS\_LAMBDA\_LOG\_FORMAT:-TEXT}"&gt;&gt;
&lt;&lt;LambdaTextFormat&gt;&gt;
&lt;&lt;PatternLayout&gt;&gt;
...
&lt;/Appenders&gt;
&lt;Loggers&gt;
&lt;Root level="${env:AWS\_LAMBDA\_LOG\_LEVEL:-INFO}"&gt;&gt;
...
```
The build configuration takes runtime dependencies on the Lambda appender and SLF4J adapter, and implementation
...
Follow the steps below to download, build, and deploy a sample Hello World Java application with integrated [Powertools for AWS Lambda (Java)](https://docs.aws.amazon.com/powertools/java/latest/) modules using the AWS SAM. This application implements a
...
```
2025/09/03/[$LATEST]851411a899b545eea2cffeba4cfbec81 2023-02-03T09:24:34.095000 INIT\_START Runtime Version: java:11.v15 Runtime Version ARN: arn:aws:lambda:eu-central-1::runtime:0a25e3e7a1cc9ce404bc435eeb2ad358d8fa64338e618d0c224fe509403583ca
2025/09/03/[$LATEST]851411a899b545eea2cffeba4cfbec81 2023-02-03T09:24:34.114000 Picked up JAVA\_TOOL\_OPTIONS: -XX:+TieredCompilation -XX:TieredStopAtLevel=1
...
"function\_request\_id": "7fcf1548-d2d4-41cd-a9a8-6ae47c51f765",
...
"executionEnvironment": "AWS\_Lambda\_java11"
}
...
"function\_request\_id": "7fcf1548-d2d4-41cd-a9a8-6ae47c51f765",
"traceId": "Root=1-63dcd2d1-25f90b9d1c753a783547f4dd;Parent=e29684c1be352ce4;Sampled=1",
"xray\_trace\_id": "1-63dcd2d1-25f90b9d1c753a783547f4dd",
"functionVersion": "$LATEST",
"Service": "service\_undefined",
"logStreamId": "2025/09/03/[$LATEST]851411a899b545eea2cffeba4cfbec81",
"executionEnvironment": "AWS\_Lambda\_java11"
...
## Sample logging code
The GitHub repository for this guide includes sample applications that demonstrate the use of various logging
...
application uses a custom `TestLogger` class that implements the `LambdaLogger` interface
with a Log4j2 logger. It uses SLF4J as a facade for compatibility with the AWS SDK. Logging libraries are
