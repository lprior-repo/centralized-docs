---
url: https://docs.aws.amazon.com/lambda/latest/dg/monitoring-cloudwatchlogs-loggroups.html
title: Configuring CloudWatch log groups
word_count: 1099
filtered: true
elements_removed: 0
density_score: 0.87
...
Configuring CloudWatch log groups - AWS Lambda
Configuring CloudWatch log groups - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#monitoring-cloudwatchlogs-loggroups)
[Execution role permissions](#monitoring-cloudwatchlogs-configure-permissions)[CloudWatch logging for Lambda Managed Instances](#monitoring-cloudwatchlogs-lmi)
...
```
`YYYY/MM/DD/&lt;&lt;function\_name&gt;&gt;[&lt;&lt;function\_version&gt;&gt;][&lt;&lt;execution\_environment\_GUID&gt;&gt;]`
```
Note that when configuring a custom log group, the name you select for your log group must follow the [CloudWatch Logs naming rules](https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_CreateLogGroup.html).
...
For your function to send logs to CloudWatch Logs, it must have the [logs:PutLogEvents](https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_PutLogEvents.html) permission.
...
When using [Lambda Managed Instances](./lambda-managed-instances.html), there are additional considerations for sending logs to CloudWatch Logs:
...
* **AWS PrivateLink (recommended)**: Use [AWS PrivateLink](https://docs.aws.amazon.com/vpc/latest/privatelink/what-is-privatelink.html)
...
an internet gateway or NAT gateway. For more information, see [Using CloudWatch Logs with interface VPC endpoints](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/cloudwatch-logs-and-interface-VPC.html).
...
* **Implement structured JSON logging**: If you're building a [custom runtime](./runtimes-custom.html)
...
| sort @timestamp asc`
```
For more information about Lambda Managed Instances logging requirements, see [Understanding the Lambda Managed Instances execution environment](./lambda-managed-instances-execution-environment.html).
