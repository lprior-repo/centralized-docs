...
title: Using the Lambda Extensions API to create extensions
word_count: 3024
filtered: true
elements_removed: 0
density_score: 0.84
...
Using the Lambda Extensions API to create extensions - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#runtimes-extensions-api)
[Lambda execution environment
lifecycle](#runtimes-extensions-api-lifecycle)[Extensions API reference](#runtimes-extensions-registration-api)
...
observability, security, and governance. Function authors can use extensions from AWS, [AWS Partners](./extensions-api-partners.html), and
open-source projects. For more information on using extensions, see [Introducing AWS Lambda Extensions](https://aws.amazon.com/blogs/aws/getting-started-with-using-your-favorite-operational-tools-on-aws-lambda-extensions-are-now-generally-available/) on the AWS
...
![The Extensions API and the Telemetry API connect Lambda and external extensions.](https://docs.aws.amazon.com/images/lambda/latest/dg/images/telemetry-api-concept-diagram.png)
As an extension author, you can use the Lambda Extensions API to integrate deeply into the Lambda [execution environment](./lambda-runtime-environment.html). Your extension can register for function and execution
...
the [Runtime Logs API](./runtimes-logs-api.html) to receive a stream of logs.
...
extension is a self-contained binary that is compatible with supported runtimes. All [Lambda runtimes](./lambda-runtimes.html) support extensions. If you use a non-compiled language, ensure that you include
...
use language-specific [environment variables and wrapper scripts](./runtimes-modify.html). You can use
these to configure the runtime environment and modify the startup behavior of the runtime process.
You can add extensions to a function in two ways. For a function deployed as a [.zip file archive](./configuration-function-zip.html), you deploy your extension as a [layer](./chapter-layers.html). For a function defined as a container image, you add [the extensions](./extensions-configuration.html#invocation-extensions-images) to your container image.
###### Note
For example extensions and wrapper scripts, see [AWS Lambda Extensions](https://github.com/aws-samples/aws-lambda-extensions) on
the AWS Samples GitHub repository.
###### Topics
* [Lambda execution environment
lifecycle](#runtimes-extensions-api-lifecycle)
...
![Lambda execution environment lifecycle for extensions](https://docs.aws.amazon.com/images/lambda/latest/dg/images/Overview-Full-Sequence.png)
###### Topics
...
* [Permissions and
configuration](#runtimes-extensions-registration-api-e)
...
phase.
![Sequence of events in the Lambda Init phase](https://docs.aws.amazon.com/images/lambda/latest/dg/images/Init-Phase.png)
### Invoke phase
...
![Sequence of events in the Lambda Invoke phase](https://docs.aws.amazon.com/images/lambda/latest/dg/images/Invoke-Phase.png)
...
they are completed.
###### Note
Lambda allocates CPU power in proportion to the function's memory setting. You might see increased
...
function metrics, see [Using CloudWatch metrics with Lambda](./monitoring-metrics.html).
...
![Sequence of events in the Lambda Shutdown phase](https://docs.aws.amazon.com/images/lambda/latest/dg/images/Shutdown-Phase.png)
...
###### Note
Because the function code directory is read-only, extensions cannot modify the
function code.
**Environment variables**: Extensions can access the
function's [environment variables](./configuration-envvars.html), except for
...
![Execution environment example: Init, Invoke, Invoke with Error, Invoke, Shutdown](https://docs.aws.amazon.com/images/lambda/latest/dg/images/Overview-Invoke-with-Error.png)
For a more detailed explanation of the previous diagram, see
[Failures during the invoke phase](./lambda-runtime-environment.html#runtimes-lifecycle-invoke-with-errors).
...
`AWS\_LAMBDA\_RUNTIME\_API` environment variable. To send a `Register`
request, use the prefix `2020-01-01/` before each API path. For example:
```
`http://${AWS\_LAMBDA\_RUNTIME\_API}/2020-01-01/extension/register `
```
###### API methods
