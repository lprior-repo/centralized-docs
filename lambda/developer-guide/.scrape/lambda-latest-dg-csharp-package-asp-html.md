---
url: https://docs.aws.amazon.com/lambda/latest/dg/csharp-package-asp.html
title: Deploy ASP.NET applications
word_count: 949
filtered: true
elements_removed: 0
density_score: 0.83
...
Deploy ASP.NET applications - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#csharp-package-asp)
[Prerequisites](#csharp-package-asp-prerequisites)[Deploying an ASP.NET Web API to Lambda](#csharp-package-asp-deploy-api)[Deploying ASP.NET minimal APIs to Lambda](#csharp-package-asp-deploy-minimal)
# Deploy ASP.NET applications
As well as hosting event-driven functions, you can also use .NET with Lambda to host lightweight ASP.NET applications. You can build and
deploy ASP.NET applications using the `Amazon.Lambda.AspNetCoreServer` NuGet package. In this section, you learn how to deploy an
...
To deploy a web API using ASP.NET, you can use the .NET Lambda templates to create a new web API project.
...
└── appsettings.json`
```
When Lambda invokes your function, the entry point it uses is the `LambdaEntryPoint.cs` file. The file created
by the .NET Lambda template contains the following code.
```
...
```
The entry point used by Lambda must inherit from one of the three base classes in the `Amazon.Lambda.AspNetCoreServer`
package. These three base classes are:
* `APIGatewayProxyFunction`
* `APIGatewayHttpApiV2ProxyFunction`
* `ApplicationLoadBalancerFunction`
The default class used when you create your `LambdaEntryPoint.cs` file using the provided .NET Lambda template is
...
of the [handler string](./csharp-handler.html#csharp-class-library-handlers) Lambda uses to invoke your function. The
...
[Name type](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-name.html) in the *CloudFormation User Guide*
to learn more.
## Deploying ASP.NET minimal APIs to Lambda
To deploy an ASP.NET minimal API to Lambda, you can use the .NET Lambda templates to create a new minimal API project. Use the following
command to initialize a new minimal API project. In this example, we name the project `MinimalApiOnLambda`.
```
