---
url: https://docs.aws.amazon.com/lambda/latest/dg/services-dynamodb-errors.html
title: Retain discarded records for a DynamoDB event source in Lambda
word_count: 1453
filtered: true
elements_removed: 0
density_score: 0.86
...
Retain discarded records for a DynamoDB event source in Lambda - AWS Lambda
Retain discarded records for a DynamoDB event source in Lambda - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#services-dynamodb-errors)
[On-failure destinations](#dynamodb-on-failure-destination-console)
...
the [get-event-source-mapping](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/lambda/get-event-source-mapping.html) command to view the current status.
...
The following example shows an invocation record Lambda sends to an S3 bucket for a DynamoDB stream. In addition to all of the fields from the previous example for SQS and SNS destinations, the `payload` field
