---
url: https://docs.aws.amazon.com/lambda/latest/dg/scheduled-task-app.html
title: Create an app to perform scheduled database maintenance
word_count: 4120
filtered: true
elements_removed: 0
density_score: 0.83
...
Create an app to perform scheduled database maintenance - AWS Lambda
Create an app to perform scheduled database maintenance - AWS Lambda
[](https://docs.aws.amazon.com/pdfs/lambda/latest/dg/lambda-dg.pdf#scheduled-task-app)
[Prerequisites](#scheduled-task-app-prereqs)[Downloading the example app files](#scheduled-task-app-download)[Creating and populating the example DynamoDB table](#scheduled-task-app-create-table)[Creating the scheduled-maintenance app](#scheduled-task-app-create-app)[Testing the app](#scheduled-task-app-test-app)[Next steps](#scheduled-task-app-next-steps)
...
![Diagram showing flow of data between an EventBridge Scheduler schedule, a Lambda function and a DynamoDB table](https://docs.aws.amazon.com/images/lambda/latest/dg/images/ExampleApps/cron_app.png)
###### Tip
...
Follow the [installation instructions](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/install-sam-cli.html)
in the *AWS SAM User Guide*.
* **AWS CLI**
...
the [installation instructions](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) in the *AWS Command Line Interface User Guide*.
* **Docker**
To deploy the app using AWS SAM, Docker must also be installed on your build machine. Follow the instructions in [Install Docker Engine](https://docs.docker.com/engine/install/)
...
* `sample\_data.csv` - a CSV file containing sample data to load into your table
* `load\_sample\_data.py` - a Python script that writes the data in the CSV file into the table
**Scheduled-maintenance app files**
* `lambda\_function.py` - the Python function code for the Lambda function that performs the database maintenance
...
**Test file**
* `test\_app.py` - a Python script that scans the table and confirms successful operation of your function by outputting all records older than one year
...
Description: SAM Template for DynamoDB Table with Order\_number as Partition Key and Date as Sort Key
Resources:
MyDynamoDBTable:
...
This AWS SAM template defines the DynamoDB table resource you create to test your app. The table uses a primary key of `Order\_number` with a sort
key of `Date`. In order for your Lambda function to find items directly by date, we also define a [Global Secondary Index](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/GSI.html)
named `Date-index`.
To learn more about creating and configuring a DynamoDB table using the `AWS::DynamoDB::Table` resource, see [AWS::DynamoDB::Table](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-dynamodb-table.html) in
the *AWS CloudFormation User Guide*.
Copy and paste the following code into a file named `sample\_data.csv`.
```
`Date,Order\_number,CustomerName,ProductID,Quantity,TotalAmount
...
Copy and paste the following code into a file named `load\_sample\_data.py`.
```
`import boto3
import csv
...
def load\_data\_from\_csv(filename):
with open(filename, 'r') as file:
csv\_reader = csv.DictReader(file)
for row in csv\_reader:
item = {
'Order\_number': row['Order\_number'],
'Date': row['Date'],
...
table.put\_item(Item=item)
print(f"Added item: {item['Order\_number']} - {item['Date']}")
if \_\_name\_\_ == "\_\_main\_\_":
load\_data\_from\_csv('sample\_data.csv')
...
table\_name = 'MyOrderTable'
table = dynamodb.Table(table\_name)
# Calculate the date one year ago
one\_year\_ago = (today - timedelta(days=365)).strftime('%Y-%m-%d')
# Scan the table using a global secondary index
response = table.scan(
IndexName='Date-index',
FilterExpression='#date &lt;&lt; :one\_year\_ago',
ExpressionAttributeNames={
'#date': 'Date'
},
ExpressionAttributeValues={
':one\_year\_ago': one\_year\_ago
}
)
# Delete old items
with table.batch\_writer() as batch:
for item in response['Items']:
Order\_number = item['Order\_number']
batch.delete\_item(
Key={
...
logger.info(f'deleted order number {Order\_number}')
...
```
The Python function code contains the [handler function](./python-handler.html) (`lambda\_handler`) that Lambda runs when your function is
invoked.
...
Copy and paste the following code into a file named `test\_app.py`.
```
`import boto3
from datetime import datetime, timedelta
import json
# Specify your table name
table\_name = 'YourTableName'
table = dynamodb.Table(table\_name)
# Calculate the date one year ago
one\_year\_ago = current\_date - timedelta(days=365)
...
ExpressionAttributeValues={
':one\_year\_ago': one\_year\_ago\_str
}
)
# The total number of old records should be zero.
print(f"Total number of old records: {len(old\_records)}")
`
```
...
2. For **Partition key**, enter `Order\_number` and leave the type as **String**.
...
5. When your table has finished creating and its **Status** shows as **Active**, create a global secondary index (GSI) by doing the
...
```
4. You can additionally confirm that the DynamoDB table has been created by opening the [Tables](https://console.aws.amazon.com/dynamodbv2/home#tables) page of the DynamoDB console.
You should see a table named `MyOrderTable`.
After you've created your table, you next add some sample data to test your app. The CSV file `sample\_data.csv` you downloaded
...
`load\_sample\_data.py` to add this data to your table.
###### To add the sample data to the table
1. Navigate to the directory containing the `sample\_data.csv` and `load\_sample\_data.py` files. If these files are in
...
5. After your function is created, you can configure your function with the provided function code.
1. In the **Code source** pane, replace the Hello world code that Lambda created with the Python function code from
the `lambda\_function.py` file that you saved earlier.
...
make it easier to search and filter log data. To learn more about Lambda log configuration options, see [Configuring advanced logging controls for Lambda functions](./monitoring-logs.html#monitoring-cloudwatchlogs-advanced).
1. Select the **Configuration** tab for your function.
...
[execution role](./lambda-intro-execution-role.html) defining the necessary permissions.
...
* Choose **Next**.
* Configure the trigger for your Lambda function by doing the following:
...
5. Choose **Next**.
6. Review your settings and choose **Create schedule** to complete creation of the schedule and Lambda trigger.
AWS SAM
...
2. Copy the `lambda\_function.py` and `requirements.txt` files you downloaded earlier to the same folder. The code location specified in the
...
We use it here so you don't need to have Python 3.12 installed on your local machine for the build to work.
...
```
5. You can additionally confirm that the Lambda function has been created by opening the [Functions](https://console.aws.amazon.com/lambda/home#/functions) page of the Lambda console.
...
After the schedule runs and invokes its target, you run the `test\_app.py` script to verify that your function successfully removed all old records
from the DynamoDB table.
###### To verify that old records are deleted using a Python script
1.
In your command line, navigate to the folder where you saved `test\_app.py`.
2. Run the script.
```
``python test\_app.py``
```
