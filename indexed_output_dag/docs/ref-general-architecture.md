---
id: ref/general/architecture
title: ARCHITECTURE
category: ref
tags: ["(presentation", "architecture:", "design", "handlers", "hexagonal"]
---

# Architecture: Hexagonal Design with Pure GoFr

> **Context**: **Location**: `internal/handlers/`
...
```go
// ❌ FORBIDDEN: Error as return value
func (a Application) Save(doc Document) error

// ✅ REQUIRED: Result[T] wraps both
...
├── cmd/docmgr/
│   └── main.go               # CLI entry point (bootstrap only)
├── pkg/domain/
...
│   │   ├── search/simple.go
│   │   ├── export/markdown.go, html.go, json.go
│   │   └── import/scanner.go
│   ├── handlers/
│   │   ├── import.go
│   │   ├── search.go
...
│       ├── logging.go         # High-cardinality logging
│       └── tracing.go         # Span helpers
├── docs/
│   ├── VISION.md
│   ├── ARCHITECTURE.md
│   ├── AGENTS.md
...
**Key Principle**: Every decision flows from "how do we make this maximally testable and observable?"
## See Also
