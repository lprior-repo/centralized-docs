    ...
    for event in events {
        match event {
            ...
            Event::Start(Tag::CodeBlock(_)) => {
                in_code_block = true;
                fixed_events.push(Event::Start(Tag::CodeBlock(Default::default())));
            }
            ...
            Event::Start(Tag::Heading(level, id, classes, attrs)) if !in_code_block => {
                ...
                let final_level = if (new_level as u32) > 4 {
                    from_u32_level(4)
                } else {
                    new_level
                };

                last_heading_level = Some(final_level as u32);
                fixed_events.push(Event::Start(Tag::Heading(final_level, id, classes, attrs)));
            }
...
fn rewrite_links_ast(
    ...
    for event in events {
        match event {
            ...
            Event::Start(Tag::CodeBlock(_)) => {
                in_code_block = true;
                transformed_events.push(Event::Start(Tag::CodeBlock(Default::default())));
            }
            Event::End(TagEnd::CodeBlock) => {
                in_code_block = false;
            ...
            Event::Start(Tag::Link(link_type, url, title)) if !in_code_block => {
                ...
                transformed_events.push(Event::Start(Tag::Link(link_type, new_url, title)));
            }
...
fn ensure_h1_ast(content: &mut String, title: &str) {
    ...
    let has_h1 = events.iter().any(|e| {
        matches!(
            e,
            Event::Start(Tag::Heading(pulldown_cmark::HeadingLevel::H1, _, _, _))
        )
    });
...
fn inject_context_block_ast(content: &mut String, context_text: &str) {
    ...
    for (i, event) in events.iter().enumerate() {
            ...
            )
        {
            ...
            new_events.push(Event::End(TagEnd::BlockQuote));
            new_events.push(Event::SoftBreak);
            new_events.push(Event::SoftBreak);
            inserted = true;
        }
...
/// Convert events to markdown (simple reconstruction)
fn events_to_markdown(events: Vec<Event>) -> String {
    ...
    for event in events {
        match event {
            ...
            Event::Start(Tag::Heading(level, _, _, _)) => {
...
mod tests {
    ...
    fn test_heading_level_conversion() {
        assert_eq!(from_u32_level(1) as u32, 1);
        assert_eq!(from_u32_level(4) as u32, 4);
        assert_eq!(from_u32_level(6) as u32, 6);
        assert_eq!(from_u32_level(10) as u32, 6); // Clamps to H6
    }
