...
/// Validate that output directory is writable before processing begins
/// This should be called BEFORE any file processing to fail fast
pub fn validate_output_dir(path: &Path) -> Result<()> {
    ...
    std::fs::write(&test_file, "test")
        .with_context(|| format!("Output directory is not writable: {}", path.display()))?;
    std::fs::remove_file(&test_file).ok(); // Clean up, ignore errors
...
fn validate_file_detailed(filename: &str, content: &str) -> Vec<ValidationIssue> {
    ...
    if word_count < 50 {
        issues.push(ValidationIssue {
            file: filename.to_string(),
            rule_id: "V009".to_string(),
            severity: Severity::Warning,
            message: format!("Document has only {} words (minimum: 50)", word_count),
            line: None,
        });
    }
