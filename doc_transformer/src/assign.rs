...
pub fn assign_ids(analyses: Vec<Analysis>) -> Result<(Vec<Analysis>, HashMap<String, IdMapping>)> {
    ...
    for analysis in &analyses {
        ...
        let filename_stem = Path::new(&analysis.source_path)
            .file_stem()
            .ok_or_else(|| anyhow::anyhow!("Invalid file path (no file stem): {}", analysis.source_path))?
            .to_string_lossy()
            .to_string();
