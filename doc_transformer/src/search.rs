...
pub fn search_index(
    ...
    for (_score, doc_address) in top_docs {
        ...
        let id = retrieved_doc
            .get_first(fields.id)
            .map(|v| tantivy::schema::OwnedValue::from(v)).and_then(|v| v.as_str().map(|s| s.to_string()))
            .map(|s| s.to_string())
        ...
        let word_count = retrieved_doc
            ...
            .map(|v| tantivy::schema::OwnedValue::from(v)).and_then(|v| v.as_u64())
            .unwrap_or(0);
        ...
        results.push(SearchResult {
            ...
            category,
            score,
            path: format!("docs/{}", id), // Placeholder, should come from index
        });
    }
