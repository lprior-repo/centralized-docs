---
id: ops/general/benchmark-spec
title: BENCHMARK SPEC
category: ops
tags: ["benchmark", "centralized-docs-8lg", "contracts", "domain", "ops"]
---

# HNSW Performance Benchmark Specification


> **Context**: Missing benchmark for HNSW performance at scale. This document specifies the complete benchmark suite with contracts, test data generators, and expect
...
MetricDescriptionTarget**Time per N**Wall-clock time to build DAG< 1s (100 chunks), < 5s (1K), < 15s (10K)**Scaling Factor**Time(2N) / Time(N)< 2.5x (sub-quadratic proof)**Edges per second**(edges_count / execution_time_ms)Higher is better**Memory usage**Peak RSS during buildProportional to N, no spikes### Design by Contract (DbC)
```
Preconditions:
...
### Handled Scenarios
ScenarioNExpected BehaviorValidation**Tiny**100Very fast, minimal edges< 100ms**Small**1,000Quick, linear scaling100-500ms**Medium**5,000Moderate time, O(n log n) visible1-5 seconds**Large**10,000Scales linearly, measurable trend5-20 seconds**Extra-large**20,000Proves scaling up to limit20-60 seconds### Boundary Conditions
...
- **Compiles successfully** (awaiting lib.rs fixes)

- **Runs without errors** for all N ∈ [100, 1K, 5K, 10K]

- **Shows sub-quadratic scaling** (doubling N increases time by < 2.5x)

- **Meets performance targets:**- 100 chunks: < 200ms
...
- **Generates HTML report** with trend graphs

- **Detects regressions** if DAG build becomes slower
