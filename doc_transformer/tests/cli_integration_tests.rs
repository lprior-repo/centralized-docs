...
fn run_cli(args: &[&str]) -> std::process::Output {
    ...
    if binary.file_name().unwrap_or_default() == "cargo" {
        ...
        Command::new(&binary)
            .args(args)
            .output()
            .expect(&format!("Failed to execute binary: {}", binary.display()))
    }
}
...
fn test_search_basic() {
    ...
    assert!(
        ...
        "Search should find results containing 'rust'. Output: {}",
        stdout
    );
}
...
fn test_scrape_invalid_url() {
    ...
    assert!(
        ...
        "Should show URL-related error. Got: {}",
        stderr
    );
}
...
fn test_ingest_git_help() {
    ...
    assert!(
        ...
        "Help should mention git functionality. Got: {}",
        stdout
    );
}
...
fn test_cli_version() {
    ...
    assert!(
        ...
        "Version should show program name or version number. Got: {}",
        stdout
    );
}
