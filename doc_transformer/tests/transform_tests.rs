...
#[test]
fn test_transform_tracks_skipped_documents() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let base_path = temp_dir.path();
    let output_dir = base_path.join("output");
    fs::create_dir_all(&output_dir).expect("Failed to create output dir");
    ...
    // Create test files
    fs::write(base_path.join("doc1.md"), "# Doc 1\nContent 1").expect("Failed to write doc1.md");
    fs::write(base_path.join("doc2.md"), "# Doc 2\nContent 2").expect("Failed to write doc2.md");
    fs::write(base_path.join("doc3.md"), "# Doc 3\nContent 3").expect("Failed to write doc3.md");

    let (files, _) = discover_files(base_path).expect("Failed to discover");
    let analyses = analyze_files(&files, base_path).expect("Failed to analyze");

    // Create link_map with only entries for doc1 and doc3 (doc2 is missing)
    let mut link_map = HashMap::new();
    for analysis in &analyses {
        let filename = analysis.source_path.split('/').last().unwrap_or("unknown");
        if filename == "doc1.md" || filename == "doc3.md" {
            let mapping = IdMapping {
                id: format!("test-{}", filename.replace(".md", "")),
                filename: format!("test-{}", filename),
                subcategory: "general".to_string(),
    ...
    assert_eq!(result.success_count, 2, "Success count should be 2 (doc1 and doc3)");
    assert_eq!(result.skipped_count, 1, "Skipped count should be 1 (doc2)");
    assert_eq!(result.error_count, 0, "Error count should be 0");

    // Verify files were created for doc1 and doc3 but not doc2
    let docs_dir = output_dir.join("docs");
    assert!(docs_dir.join("test-doc1.md").exists(), "doc1 should be created");
    assert!(docs_dir.join("test-doc3.md").exists(), "doc3 should be created");
    assert!(!docs_dir.join("test-doc2.md").exists(), "doc2 should NOT be created");
}

#[test]
fn test_transform_no_skipped_when_all_mapped() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let base_path = temp_dir.path();
    let output_dir = base_path.join("output");
    fs::create_dir_all(&output_dir).expect("Failed to create output dir");
    ...
    // Create link_map with entry for doc1
    let mut link_map = HashMap::new();
    for analysis in &analyses {
        let filename = analysis.source_path.split('/').last().unwrap_or("unknown");
        let mapping = IdMapping {
    ...
    // Verify no documents were skipped
...
#[test]
fn test_transform_empty_link_map_skips_all() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let base_path = temp_dir.path();
    let output_dir = base_path.join("output");
    fs::create_dir_all(&output_dir).expect("Failed to create output dir");
    ...
    // Verify all documents were skipped
    ...
    let docs_dir = output_dir.join("docs");
    if docs_dir.exists() {
        let entries: Vec<_> = fs::read_dir(&docs_dir).unwrap().collect();
        assert_eq!(entries.len(), 0, "No documents should be created");
    }
}
