...
fn test_long_document_splits_on_token_limit() {
    ...
    assert!(
        chunks.len() > 1,
        "Long content should create multiple chunks at summary level"
    );
...
fn test_unicode_emoji_handling() {
    ...
    assert!(
        combined.contains("🎉") || combined.contains("Document"),
        "Content lost during chunking"
    );
}
...
fn test_cjk_text_chunking() {
    ...
    assert!(
        combined.contains("文档") || combined.contains("中文"),
        "Chinese characters lost"
    );
}
...
fn test_empty_document_handling() {
    ...
    assert!(
        !chunks.is_empty(),
        "Should create at least one chunk for empty content"
    );
...
fn test_multiple_h2_headers_create_chunks() {
    ...
    assert!(
        chunks.len() >= 3,
        "Should create at least 3 chunks for 3 H2 sections"
    );
...
fn test_chunk_id_format() {
    ...
    for chunk in &chunks {
        ...
        assert!(
            chunk.chunk_id.starts_with("test-doc#"),
            "Chunk ID should start with doc-id#"
        );
    }
}
