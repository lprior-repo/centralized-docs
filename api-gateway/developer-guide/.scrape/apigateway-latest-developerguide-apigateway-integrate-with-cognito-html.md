...
title: Control access to REST APIs using
word_count: 677
filtered: true
elements_removed: 0
density_score: 0.83
...
Control access to REST APIs using Amazon Cognito user pools as an authorizer - Amazon API Gateway
[](https://docs.aws.amazon.com/pdfs/apigateway/latest/developerguide/apigateway-dg.pdf#apigateway-integrate-with-cognito)
# Control access to REST APIs using
Amazon Cognito user pools as an authorizer
As an alternative to using [IAM roles and policies](./permissions.html) or
[Lambda authorizers](./apigateway-use-lambda-authorizer.html) (formerly
...
`COGNITO\_USER\_POOLS` type and then configure an API method to use that
...
authorization type, category `Method Execution`, to `AWS\_IAM`.
...
###### Topics
* [Create an Amazon Cognito user pool for a REST API](./apigateway-create-cognito-user-pool.html)
* [Integrate a REST API with an Amazon Cognito user
pool](./apigateway-enable-cognito-user-pool.html)
* [Call a REST API
integrated with an Amazon Cognito user pool](./apigateway-invoke-api-integrated-with-cognito-user-pool.html)
...
* [ Create an Amazon Cognito authorizer for a REST API using CloudFormation](./apigateway-cognito-authorizer-cfn.html)
