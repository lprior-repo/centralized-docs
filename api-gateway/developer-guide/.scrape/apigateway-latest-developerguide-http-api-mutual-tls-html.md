---
url: https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-mutual-tls.html
title: How to turn on mutual TLS authentication for your
word_count: 1793
filtered: true
elements_removed: 0
density_score: 0.86
...
How to turn on mutual TLS authentication for your HTTP APIs in API Gateway - Amazon API Gateway
How to turn on mutual TLS authentication for your HTTP APIs in API Gateway - Amazon API Gateway
[](https://docs.aws.amazon.com/pdfs/apigateway/latest/developerguide/apigateway-dg.pdf#http-api-mutual-tls)
[Prerequisites for mutual TLS](#http-api-mutual-tls-prerequisites)[Configuring mutual TLS for a custom
domain name](#http-api-mutual-tls-configure)[Invoke an API by using a custom domain
name that requires mutual TLS](#http-api-mutual-tls-invoke)[Updating your truststore](#http-api-mutual-tls-update-truststore)[Disable mutual TLS](#http-api-mutual-tls-disable)[Troubleshoot mutual TLS for your HTTP API](#http-api-mutual-tls-troubleshooting)
...
authentication operations](./apigateway-control-access-to-api.html) that API Gateway supports. API Gateway forwards the certificates that clients provide to
...
`execute-api` endpoint. To learn more, see [Disable the default endpoint for HTTP APIs](./http-api-disable-default-endpoint.html).
...
### Custom domain names
To enable mutual TLS for a HTTP API, you must configure a custom domain name for your API. You
...
in API requests. You can find more information at [Custom domain names for HTTP APIs in API Gateway](./http-api-custom-domain-names.html).
...
certificates. To setup a certificate in ACM, go to [ACM](https://console.aws.amazon.com/acm/). If you would like to import a
certificate, continue reading in the following section.
...
chain length of four. You can also provide self-signed certificates. The following hashing algorithms are
...
|
The names and subjects of certificates must form an unbroken
chain. Certificates can have a maximum chain length of four.
|
...
...`
```
The following [cp](https://docs.aws.amazon.com/cli/latest/reference/s3/cp.html) AWS CLI command uploads
...
After you've uploaded your truststore to Amazon S3, you can configure your custom domain
name to use mutual TLS. The following [create-domain-name](https://docs.aws.amazon.com/cli/latest/reference/apigatewayv2/create-domain-name.html) creates a
...
The following conditions will cause API Gateway to fail the TLS handshake and deny the request with a `403` status code. If your certificate:
* isn't trusted
* is expired
...
Use [Amazon S3
versioning](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html) to maintain multiple versions of your truststore. When you update
...
The following [update-domain-name](https://docs.aws.amazon.com/cli/latest/reference/apigatewayv2/update-domain-name.html) command
...
```
## Disable mutual TLS
To disable mutual TLS for a custom domain name, remove the truststore from your custom
domain name, as shown in the following command.
The following [update-domain-name](https://docs.aws.amazon.com/cli/latest/reference/apigatewayv2/update-domain-name.html)
command updates a custom domain name to remove the truststore from your custom domain name:
```
`aws apigatewayv2 update-domain-name \\
--domain-name api.example.com \\
--domain-name-configurations CertificateArn=arn:aws:acm:`us-west-2`:`123456789012`:certificate/`123456789012-1234-1234-1234-12345678` \\
...
The error `"The certificate subject &lt;certSubject&gt; conflicts with an existing certificate from a
different issuer."` means multiple Certificate Authorities have issued a certificate for this domain. For
each subject in the certificate, there can only be one issuer in API Gateway for mutual TLS domains. You will need to
get all of your certificates for that subject through a single issuer. If the problem is with a certificate you
don't have control of but you can prove ownership of the domain name, [contact Support](https://console.aws.amazon.com/support/cases#/create) to open a ticket.
### Troubleshooting domain name status messages
`PENDING\_CERTIFICATE\_REIMPORT`: This means you reimported a certificate to ACM and it failed
...
valid certificate into ACM. For more information about validation see [Validating domain ownership](https://docs.aws.amazon.com/acm/latest/userguide/domain-ownership-validation.html).
`PENDING\_OWNERSHIP\_VERIFICATION`: This means your previously verified certificate has expired and
...
renewal](https://docs.aws.amazon.com/acm/latest/userguide/troubleshooting-renewal.html) guide.
