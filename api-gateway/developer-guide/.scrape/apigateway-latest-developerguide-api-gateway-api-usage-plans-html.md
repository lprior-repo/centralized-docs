...
Usage plans and API keys for REST APIs in API Gateway - Amazon API Gateway
[](https://docs.aws.amazon.com/pdfs/apigateway/latest/developerguide/apigateway-dg.pdf#api-gateway-api-usage-plans)
[What are usage plans and API
keys?](#api-gateway-api-usage-plans-overview)[Best practices for API keys and usage plans](#apigateway-usage-plans-best-practices)
...
together with [Lambda authorizers](./apigateway-use-lambda-authorizer.html), [IAM roles](./permissions.html), or
[Amazon Cognito](./apigateway-integrate-with-cognito.html) to control
access to your APIs. API Gateway can generate API keys on your behalf, or you can import them
from a [CSV file](./api-key-file-format.html). You can generate an API key
in API Gateway, or import it into API Gateway from an external source. For more information, see
[Set up API keys for REST APIs in API Gateway](./api-gateway-setup-api-keys.html).
...
A *throttling limit* sets the target point at which request throttling should start. This
can be set at the API or API method level.
A *quota limit* sets the target maximum number of requests with a given API key that can be
...
*all* APIs in that usage plan. Instead, to control access to your API, use an IAM
role, a [Lambda authorizer](./apigateway-use-lambda-authorizer.html), or an [Amazon Cognito user pool](./apigateway-integrate-with-cognito.html).
