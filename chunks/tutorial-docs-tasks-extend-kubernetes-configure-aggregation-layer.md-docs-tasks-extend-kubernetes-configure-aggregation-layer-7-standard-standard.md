---
doc_id: tutorial/docs-tasks-extend-kubernetes-configure-aggregation-layer.md/docs-tasks-extend-kubernetes-configure-aggregation-layer
chunk_id: tutorial/docs-tasks-extend-kubernetes-configure-aggregation-layer.md/docs-tasks-extend-kubernetes-configure-aggregation-layer#7-standard
chunk_level: standard
chunk_type: prose
heading: Enable Kubernetes Apiserver flags
token_count: 18
summary: ### CA Reusage and Conflicts The Kubernetes apiserver has two client CA options:
---

### CA Reusage and Conflicts
The Kubernetes apiserver has two client CA options: